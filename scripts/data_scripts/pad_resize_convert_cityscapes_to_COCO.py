...
def polygon_to_resize_pad(polygons):
    ...
    for ele in polygons:
        ...
        for i in range(len(ele)):
            if (i%2==0):
                ...
                odd_i.append(ele[i])
        even_i = [x * r_img for x in even_i]
        odd_i = [(y * r_img + pad_each_side) for y in odd_i]
        for i in range(len(ele)):
            if (i%2==0):
                # print(i)
                ele[i]=int(even_i[int(i/2)])
            else:
                # print(i)
                ele[i]=int(odd_i[int((i-1)/2)])
    return polygons
            ...
            if id2label[instanceObj.labelID].hasInstances:
                ...
                for ele in polygons:
                    for i in ele:
                        ...
                        i = i*r_img
                '''
                polygons = polygon_to_resize_pad(polygons)
                # print(polygons)
        ...
        for root, _, files in os.walk(ann_dir):
            for filename in files:
                if filename.endswith(ends_in % data_set.split('_')[0]):
                    ...
                    image['width'] = desire_size # json_ann['imgWidth']*r_img
                    image['height'] = desire_size # json_ann['imgHeight']*r_img
...
if __name__ == '__main__':
    ...
    print(r_img)
    y_resized = 1024*r_img
    pad_each_side = (desire_size - y_resized)/2
